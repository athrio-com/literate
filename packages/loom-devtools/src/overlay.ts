...
const control = (label: string, message: Message, tone: string): Html =>
  h.button(
    [
      h.Class(clsx('cursor-pointer rounded px-1.5 py-0.5 text-[11px] transition hover:bg-fg/5', tone)),
      h.OnClick(message),
    ],
    [label],
  )
  ...
  h.div(
    ...
    [
      h.textarea(
        [
          h.Class(
            'w-full resize-none rounded-md border border-line bg-bg px-2.5 py-2 font-sans text-[12.5px] text-fg focus:border-mint focus:outline-none',
          ),
          h.Value(editText),
      ...
      h.div(
        ...
        [
          ...
          h.button(
            [
              h.Class(
                'cursor-pointer rounded-md border border-line px-2.5 py-1 text-[11px] text-fg-2 transition hover:text-fg',
              ),
...
const bar = (model: Model): Html =>
  h.div(
    [
      h.Class(
        'pointer-events-auto fixed inset-x-0 bottom-0 flex h-8 items-center gap-2 border-t border-line bg-bg-2 pl-4 pr-3 text-[12px] text-fg-2',
      ),
    ],
    [
      h.div(
...
const handle = (model: Model): Html => {
  ...
  return h.button(
    [
      h.Class(
        'pointer-events-auto fixed bottom-0 right-6 inline-flex h-7 cursor-pointer items-center gap-1.5 rounded-t-md border-l border-r border-t border-line bg-bg-2 px-3 text-[12px] text-fg-2 transition hover:bg-bg-3 hover:text-fg',
      ),
      h.OnClick(ToggledCollapse()),
...
const draftArea = (model: Model, placeholder: string): Html =>
  h.textarea(
    [
      h.Class(
        'h-16 w-full resize-none rounded-md border border-line bg-bg px-2.5 py-2 font-sans text-[12.5px] leading-relaxed text-fg placeholder:text-fg-4 focus:border-mint focus:outline-none',
      ),
      h.Value(model.draft),
      ...
      h.Class(
        clsx(
          ...
          model.picking
            ...
            : 'border-line bg-bg-3 text-fg-2 hover:border-white/20 hover:text-fg',
        ),
      ),
      h.OnClick(ToggledPick()),
    ],
...
const scrollDown = (): Html =>
  h.button(
    [
      h.Class(
        'absolute -top-12 right-2 inline-flex h-9 w-9 cursor-pointer items-center justify-center rounded-full border border-line bg-bg-3 text-fg-2 shadow-[0_6px_20px_rgba(0,0,0,0.5)] transition hover:text-fg',
      ),
...
const composer = (model: Model): Html =>
  h.div(
    [h.Class('relative border-t border-line-2 p-3')],
    [
...
const noteList = (model: Model): Html => {
  ...
  return h.div(
    [h.Id(notesListId), h.Class('flex min-h-0 flex-1 flex-col divide-y divide-line-2 overflow-auto')],
    notes.length === 0
      ? [emptyState(model)]
...
const banner = (): Html =>
  h.div(
    [h.Class('border-b border-line-2 bg-amber/[0.08] px-3 py-2.5 font-sans text-[12px] leading-relaxed')],
    [
...
const header = (model: Model): Html =>
  h.div(
    [h.Class('flex items-center gap-4 border-b border-line-2 px-3 py-2.5 text-[13px]')],
    [
      ...
      h.button(
        [
          h.Class(
            'ml-auto flex h-6 w-6 cursor-pointer items-center justify-center rounded-md border border-line text-fg-2 transition hover:bg-fg/5 hover:text-fg',
          ),
          h.OnClick(Toggled()),
          h.Title('Close (Alt+N)'),
        ],
...
const panel = (model: Model): Html =>
  h.div(
    [
      h.Class(
        'pointer-events-auto fixed bottom-10 right-4 flex max-h-[80vh] w-[360px] flex-col overflow-hidden rounded-[10px] border border-line bg-bg-2 font-mono text-[13px] text-fg shadow-[0_18px_50px_rgba(0,0,0,0.45)]',
      ),
    ],
      ...
      h.div(
        ...
        [
          h.button(
            [
              h.Class(
                'flex-1 cursor-pointer rounded-md border border-line bg-bg-3 py-2 text-[12px] text-fg-2 transition hover:text-fg',
              ),
              h.OnClick(Escaped()),
            ],
            ['Cancel'],
...
const followTheme = (host: HTMLElement): void => {
  const apply = (): void => host.setAttribute('data-theme', pageTheme())
  apply()
